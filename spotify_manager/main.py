...
simplified_albums = [
    {
        ...
        "ordering_name": re.sub(
            r"^(?:the|a|an)?[\W\s]+", "", album["album"]["name"]
        ).upper(),
...
with open("albums_total.txt", "w") as main_file:
    for album in sorted_albums:
        ...
        # print(f"{album_name} - {album_artist_name} ({album_genre_main}): ")
        main_file.write(f"{album_name} - {album_artist_name}: \n")
